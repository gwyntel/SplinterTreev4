...
async def test_api_client_initialization():
    ...
    try:
        ...
        assert api.openai_client.base_url == f"{HELICONE_API_URL}/v1"
        assert 'Helicone-Auth' in api.openai_client.default_headers
        
        assert api.openpipe_client is not None
        assert api.openpipe_client.base_url == f"{HELICONE_API_URL}/v1"
...
async def test_openpipe_call_with_helicone():
    ...
    try:
        ...
        with patch.object(api.openpipe_client.chat.completions, 'create', 
                         new_callable=AsyncMock, side_effect=mock_create) as mock_create:
...
async def test_helicone_rate_limit_handling():
    ...
    try:
        ...
        with patch.object(api.openpipe_client.chat.completions, 'create', 
                         ...
                         side_effect=[mock_create, mock_create]) as mock_create:
